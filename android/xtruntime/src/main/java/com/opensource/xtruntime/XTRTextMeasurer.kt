...
class XTRTextMeasurer {
    ...
    class JSExports(val context: XTRContext): XTRComponentExport() {
        ...
        fun measureText(text: String, params: V8Object): V8Value {
            ...
            val font = XTMemoryManager.find(fontRef) as? XTRFont ?: return V8.getUndefined()
            ...
            XTRUtils.toRect(rectObject)?.let {
                ...
                val bounds = XTRRect(0.0, 0.0, measureTextView.measuredWidth.toDouble() / context.appContext.resources.displayMetrics.density, measureTextView.measuredHeight.toDouble() / context.appContext.resources.displayMetrics.density)
                return XTRUtils.fromRect(bounds, context.runtime)
            }
