...
class XTRBridge(val appContext: android.content.Context, val completionBlock: ((bridge: XTRBridge) -> Unit)? = null, val failureBlock: ((e: Exception) -> Unit)? = null) {
    ...
    private fun loadComponents() {
        val components: List<XTRComponentExport> = listOf(
                ...
                XTRNavigationBar.JSExports(xtrContext),
                XTRModal.JSExports(xtrContext),
                XTRWebView.JSExports(xtrContext),
                XTRSwitch.JSExports(xtrContext),
                XTRSlider.JSExports(xtrContext),
                XTRActivityIndicatorView.JSExports(xtrContext)
        )
