...
export class ViewLayer extends BaseLayer {
    ...
    backgroundColor: UI.Color = UI.Color.whiteColor
    frame?: UI.Rect = undefined
    ...
    protected findProps(): any {
        ...
        backgroundColorProp.valueChanged = (value: string) => {
            if (value.indexOf("#") === 0) {
                if (value.length == 7) {
                    try {
                        ...
                        this.backgroundColor = new UI.Color(r, g, b, 1.0)
                        this.propsDidChange && this.propsDidChange();
                    } catch (error) { }
                }
                if (value.length == 9) {
                    try {
                        ...
                        this.backgroundColor = new UI.Color(r, g, b, a)
                        this.propsDidChange && this.propsDidChange();
                    } catch (error) { }
                }
            }
        ...
        frameProp.valueChanged = (value) => {
            this.frame = UI.RectMake(value[0] || 0, value[1] || 0, value[2] || 0, value[3] || 0)
            this.propsDidChange && this.propsDidChange();
        }
        return {
            classProp,
    ...
    requestDesignView(): UI.View | undefined {
...
ViewLayer.componentCloner[ViewLayer.componentID] = (originLayer: any) => {
    const newLayer: any = new ViewLayer()
